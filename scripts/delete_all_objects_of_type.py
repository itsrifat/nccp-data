...
CERTIFICATE = '/var/local/dataone/certs/client/client_cert_local.pem'
CERTIFICATE_KEY = '/var/local/dataone/certs/client/client_key_nopassword_local.pem'
...
def main(formatId):
  ...
  member_node_object_deleter.delete_objects_from_member_node(formatId)
...
class MemberNodeObjectDeleter(object):
  def __init__(self, base_url):
    ...
    self._mn_client = d1_client.mnclient.MemberNodeClient(self._base_url,
      cert_path=CERTIFICATE, key_path=CERTIFICATE_KEY)


  def delete_objects_from_member_node(self,formatId):
    logging.info('Searching for objects to delete on Member Node: {0}'.format(self._base_url))
    pids_delete = self._find_objects_to_delete(formatId)
    logging.info('Found {0} objects to delete'.format(len(pids_delete)))
    if not len(pids_delete):
      return
    self._delete_objects(pids_delete)
    # Check for success.
    pids_remaining = self._find_objects_to_delete(formatId)
    if len(pids_remaining):
      logging.error('Deletion failed on {0} of {1} objects'
                    .format(len(pids_remaining), len(pids_delete)))
    else:
      logging.info('Successfully deleted {0} objects'.format(len(pids_delete)))


  def _find_objects_to_delete(self,formatId):
    pids = []
    current_start = 0
    while True:
      try:
        object_list = self._mn_client.listObjects(start=current_start,
          count=LIST_OBJECTS_PAGE_SIZE, objectFormat=formatId,
          replicaStatus=False)
      except d1_common.types.exceptions.DataONEException as e:
        logging.exception('listObjects() failed with exception:')
        raise

      logging.info('Retrieved page: {0}/{1} ({2} objects)'.format(
...
if __name__ == '__main__':
  if len(sys.argv)!=2:
      print 'Provide a format id as argument'
  else:
      main(str(sys.argv[1]))
